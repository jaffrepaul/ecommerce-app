...
Initial implementation hardcoded `companyId` in client-side config, making it:

- Visible in browser JavaScript bundles
- Easy to tamper with via DevTools
- Same for all users (not multi-tenant)
...
- Receives `companyId` from server as prop (secure!)
...
| ---------------- | ---------------------- | -------------------------- |
| **Storage**      | Hardcoded in JS        | From authenticated session |
| **Visibility**   | In browser source code | Runtime only               |
| **Tampering**    | Easy via DevTools      | Protected by server        |
| **Multi-tenant** | Same for all users     | Per-user from database     |
...
- `setBy` attribute (shows which method set it)
  - `CLIENT-beforeSendLog-SECURE` (client-side logs)
  - `SERVER-beforeSendLog-SECURE` (server-side logs)
  - `middleware-SECURE` (tags on all events)
